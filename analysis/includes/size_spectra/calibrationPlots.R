...
calibrationPlots<- function(state_new,param,SF,Fmsy,cuts) {# State_new data, parameters from size based model, and equilibrium simulation
...
ymin = signif(min(df$bio_obs),1)/2
ymax = signif(max(df$bio_obs),1)*2
...
p2 <- ggplot(data = df, aes(x = log10(SSBIO),y = log10(bio_obs))) + geom_point(size = 4)+
  ...
  geom_abline(intercept = 0, slope = 1)+theme(text = element_text(size=9))
...
# Plot growth
source('IterateSpectrumNG.R')
pNew <- param
#pNew$h <- param$h*1.2
SFNG <- IterateSpectrumNG(pNew,SF)

p4 <- plotAvgVonB(param,SFNG,state_new)

# Plot the catch 

for (i in 1:param$nSpecies){
  if (is.na(state_new$Catch[i]) == 0){
...
M2plot <- ggplot(data = data.frame(x = SF$w,y = M2[1,]),aes(x = x, y = y))+geom_line(alpha = 1/2)+
  scale_y_continuous(name = expression(paste(mu," (",y^-1,")")),limits = c(0,max(M2[,w>1],na.rm=T)))+
  scale_x_log10(name = 'weight (g)',
                limits = c(1,max(param$wInf)),
                breaks = c(1,10,100,1000,1e5))+
  theme_classic()+theme(text = element_text(size=9))

for (i in 2:param$nSpecies){
  M2plot <- M2plot + geom_line(data = data.frame(x = SF$w,y = M2[i,]),aes(x = x, y = y),  alpha = 1/2)
  
}

if (length(which(is.na(state_new$M) == 0)) > 0){
M2plot <- M2plot + geom_point(data = state_new, aes(x = param$alphaMature*wInf, y = M), shape = 2)
}


M1_df <- data.frame(x = param$alphaMature*t(param$wInf),y = M1)
...
dfMean <- data.frame(eidx = c(weighted.mean(dfSmall$eidx,na.rm=T),mean(dfMedium$eidx,na.rm=T),mean(dfLarge$eidx,na.rm=T)), 
                     grps = cLabels)
...
p7 <- ggplot(dfMean, aes(x=as.factor(grps),y=eidx))+geom_bar(stat='identity')+theme_classic()+
  scale_x_discrete('Size group')+
  scale_y_continuous('Exploitation',expand = c(0,0), limits = c(0,max(dfMean$eidx)))+
...
p9 <- ggplot(data=df,  aes(x=wInf,y = Rmax))+scale_y_log10('Maximum recruitment')+
  scale_x_log10('asymptotic weight (g)')+theme_classic()+geom_point()+theme(text = element_text(size=9))
...
p12 <- ggplot(data=state,aes(x=wInf,y=Fmsy))+geom_point()+
  ...
  theme_classic()+theme(text = element_text(size=9))+
  scale_x_log10('Asymptotic weight')+scale_y_continuous(limits = c(0,max(rbind(Fmsy,state$Fmsy),na.rm=T)))
p12


grid.arrange(p1,p2,p3,p4,p5,M2plot,p7,p8,p9,p10,p11,p12)

return(list(p1,p2,p3,p4,p5,M2plot,p7,p8,p9,p10,p11,p12))
}